...
class EmailProvider{
	...
	public static function sendLoginReport(string $addr, array $data){
		...
		if(!empty($addr)){
			...
			$email->setHeader('X-Priority', '3');
			$date = date('d/m/Y');
			$email->setSubject('New sign-in to your account #'.uniqid());
	        // Set headers for categorization
	...
	public static function sendSignUpReport(string $addr)
	{
		$email = \Config\Services::email();
		...
		if(!empty($addr)){
			...
			$email->setHeader('X-Priority', '3');
			$date = date('d/m/Y');
			$email->setSubject('Signup Successful');
	        // Set headers for categorization
			$email->setHeader('List-ID', 'login-alerts.codeblaze.in');
			$email->setHeader('Precedence', 'bulk');
			// $message = view('\SrvKit\Auth\Views\emails\login-alert', $_data());
			...
			<div>
				<h2>Hello,</h2>
				<p>Now you are registered to srvkit authentication.</p>
			</div>
			HTML;
			$email->setMessage($message);
			$email->send();
